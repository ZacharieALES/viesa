 ...
 *
 */
public class LPCALimitedLines{
	
	private AnnotatedArray aa1;
	private AnnotatedArray aa2;

	public Double extractionInsDelScore;
	public Double unwantedSubstitution;
	
	public static TreeSet<Position> traceback;

	
	public int linesToProcess = -1;
	...
	/** LPCA is a singleton */
	public LPCALimitedLines(){}
	
	
	private void computeR() {

		rS = new double[this.linesToProcess][aa1.getNumberOfAnnotationColumns()][this.linesToProcess];

		/* For each couple of lines in ae1 and ae2 */
		for (int i = 0; i < this.linesToProcess; i++) {
			for (int k = 0; k < this.linesToProcess; k++) {
...
//						.get(k));
	...
	private void computeC() {
		...
		cS = new double[this.linesToProcess][aa1.getNumberOfAnnotationColumns()][this.linesToProcess];
...
//					aa2.getColumn(j));
	...
	private void computeT() {
		...
		tSMemory = new ScoreTable(this.linesToProcess + 1,
				aa1.getNumberOfAnnotationColumns() + 1,
				this.linesToProcess + 1);
		tS = new ScoreTable(this.linesToProcess + 1,
				aa1.getNumberOfAnnotationColumns() + 1,
				this.linesToProcess + 1);

		/* For each other elements in tS (and tSMemory) */
		for (int i = 1; i < this.linesToProcess + 1; i++)
			for (int j = 1; j < aa1.getNumberOfAnnotationColumns() + 1; j++)
				for (int k = 1; k < this.linesToProcess + 1; k++) {

					{
						...
						/* Case 8 */
						if (cS[i - 1][j - 1][k - 1] != 0 && j != 1
								&& rS[i - 1][j - 2][k - 1] != 0)
							tempScore = tS.get(i - 1, j - 1, k - 1)
									+ cS[i - 1][j - 1][k - 1]
									+ rS[i - 1][j - 2][k - 1];
						else {
							tempScore = tS.get(i - 1, j - 1, k - 1) + 2
						...
						/* Case 6 */
						if (cS[i - 1][j - 1][k - 1] != 0)
							tempScore = tS.get(i, j - 1, k)
									+ cS[i - 1][j - 1][k - 1];
						else {
							tempScore = tS.get(i, j - 1, k) + 2
									* extractionInsDelScore;
							/*
							 * Used if insertion and deletion costs are
							 * different
						...
						/* Case 5 */
						if (rS[i - 1][j - 1][k - 1] != 0)
							tempScore = tS.get(i - 1, j, k - 1)
									+ rS[i - 1][j - 1][k - 1];
						else {
							tempScore = tS.get(i - 1, j, k - 1) + 2
									* extractionInsDelScore;
							/*
							 * Used if insertion and deletion costs are
							 * different
	...
	public ExtractionAlignments align(AnnotatedArray aa1, AnnotatedArray aa2, int desired_number_of_alignments, double minimum_score, int lineToProcess){		
		
		this.linesToProcess = lineToProcess;
	...
	private void traceback() {
		...
		LPCALimitedLines.traceback = new TreeSet<Position>(tracebackSeeds);
...
//			System.out.println(tracebackSeeds.size() + " seeds found");
		...
		while (tracebackSeeds.size() > 0) {
			...
			if(positionScore >= alignments.minimumScore){
				...
				/*
				 * Create a new ArrayList<Position> and remove the position from the
				 * seed's list
				 */
...
//					System.out.println("\tIn LPCA: number of alignments: " + alignments.currentNumberOfAlignments + " min score: " + alignments.minimumScore);
	...
	private TreeSet<Position> getTSCompatiblePositions() {
		...
		for (int j = aa1.getNumberOfAnnotationColumns(); j >= 0; j--)
			for (int i = this.linesToProcess; i >= 0; i--)
				for (int k = this.linesToProcess; k >= 0; k--) {
		...
		if (i != k) {
			...
			while (isValid && t < coord.size()) {
				...
				if (coord.get(t).getX() == coord.get(t).getY()) {
					...
					Position newPos = new Position(i + 1, coord.get(t).getX() + 1,
							k + 1);
	...
	private ArrayList<Position> getColAlignedAnnotations(int i, int j, int k) {
		...
		while (t < coord.size() && isValid) {
			...
			Position newPos = new Position(coord.get(t).getX() + 1, j + 1,
					coord.get(t).getY() + 1);
