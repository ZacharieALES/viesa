...
public class LPCA4D{
	...
	public Double extractionInsDelScore;
	private Double unwantedSubstitution;

	public static boolean doStupidTraceback = false;

	public int lineToProcess = -1;
	...
	/**
	 * For a couple of AnnotatedArray, save the columnScoreMatrix when
	 * computing cS in order to avoid recomputing them when tracing back
	 */
	transient private ArrayList<List<double[][]>> columnScoreMatrix = new ArrayList<>();
	 ...
	 * ae2[k-1,l-1]
	 */
	transient private ScoreTable4D tS;
	 ...
	 * ligne)
	 */
	transient private ScoreTable4D tSMemory;
	...
	private void computeR() {
		...
		rS = new double[this.lineToProcess][aa1.getNumberOfAnnotationColumns()][this.lineToProcess][aa2.getNumberOfAnnotationColumns()];

		/* For each couple of lines in ae1 and ae2 */
		for (int i = 0; i < this.lineToProcess; i++) {
			for (int k = 0; k < this.lineToProcess; k++) {

				double[][] scoreM = computeLocalScoreMatrix(getLine(aa1,i), getLine(aa2, k));
				...
				for (int j = 0; j < aa1.getNumberOfAnnotationColumns(); j++)
					for (int l = 0; l < aa2.getNumberOfAnnotationColumns(); l++)
						rS[i][j][k][l] = scoreM[j + 1][j + 1];
			}
		}
	}
	...
	private void computeC() {
		...
		cS = new double[this.lineToProcess][aa1.getNumberOfAnnotationColumns()][this.lineToProcess][aa2.getNumberOfAnnotationColumns()];

		/* For couple of column */
		for (int j = 0; j < aa1.getNumberOfAnnotationColumns(); j++) {

			columnScoreMatrix.add(new ArrayList<double[][]>());
			for(int l = 0 ; l < aa2.getNumberOfAnnotationColumns() ; ++l){

				/* Compute the similarity between column i of aa1 and column j of aa2 */
				double[][] simM = computeLocalScoreMatrix(aa1.getAnnotations().get(j).getValues(),
						aa2.getAnnotations().get(l).getValues());
				//			int[][] simM = computeLocalScoreMatrix(aa1.getColumn(j),
				//					aa2.getColumn(j));

				columnScoreMatrix.get(j).add(l, simM);

				/* For each couple of prefix strings in these columns */
				for (int i = 0; i < this.lineToProcess; i++)
					for (int k = 0; k < this.lineToProcess; k++)
						cS[i][j][k][l] = simM[i + 1][k + 1];
			}
		}
	}
	...
	private void computeT() {
		...
		tSMemory = new ScoreTable4D(this.lineToProcess + 1,
				aa1.getNumberOfAnnotationColumns() + 1,
				this.lineToProcess + 1, aa2.getNumberOfAnnotationColumns() + 1);
		tS = new ScoreTable4D(this.lineToProcess + 1,
				aa1.getNumberOfAnnotationColumns() + 1,
				this.lineToProcess + 1, aa2.getNumberOfAnnotationColumns() + 1);

		/* For each other elements in tS (and tSMemory) */
		for (int i = 1; i < this.lineToProcess + 1; i++)
			for (int j = 1; j < aa1.getNumberOfAnnotationColumns() + 1; j++)
				for (int k = 1; k < this.lineToProcess + 1; k++) 
					for (int l = 1; l < aa2.getNumberOfAnnotationColumns() + 1; l++) {

						{
							...
							//						System.out.println(cS.length + " " + cS[0].length + " " + cS[0][0].length + " " + cS[0][0][0].length);
							//						System.out.println(i +"," + j + "," + k + "," + l);
							/* Case 8 */
							if (cS[i - 1][j - 1][k - 1][l-1] != 0 && j != 1
									&& rS[i - 1][j - 2][k - 1][l-1] != 0)
								tempScore = tS.get(i - 1, j - 1, k - 1, l-1)
								+ cS[i - 1][j - 1][k - 1][l-1]
										+ rS[i - 1][j - 2][k - 1][l-1];
							else {
								tempScore = tS.get(i - 1, j - 1, k - 1, l-1) + 2
										* extractionInsDelScore;
								/*
							...
							/* Case 7 */
							if (i != 1 && k != 1 && cS[i - 2][j - 1][k - 2][l-1] != 0
									&& rS[i - 1][j - 1][k - 1][l-1] != 0)
								tempScore = tS.get(i - 1, j - 1, k - 1, l-1)
								+ cS[i - 2][j - 1][k - 2][l-1]
										+ rS[i - 1][j - 1][k - 1][l-1];
							else {
							...
							/* Case 6 */
							if (cS[i - 1][j - 1][k - 1][j-1]!= 0)
								tempScore = tS.get(i, j - 1, k, l-1)
								+ cS[i - 1][j - 1][k - 1][l-1];
							else {
								tempScore = tS.get(i, j - 1, k,l-1) + 2
										* extractionInsDelScore;
								/*
								 * Used if insertion and deletion costs are
								 * different
							...
							/* Case 5 */
							if (rS[i - 1][j - 1][k - 1][l-1] != 0)
								tempScore = tS.get(i - 1, j, k - 1, l)
								+ rS[i - 1][j - 1][k - 1][l-1];
							else {
								tempScore = tS.get(i - 1, j, k - 1, l-1) + 2
										* extractionInsDelScore;
								/*
								 * Used if insertion and deletion costs are
								 * different
							...
							/* Case 3 */
							tempScore = tS.get(i, j, k - 1, l) + extractionInsDelScore;
							...
							/* Case 1 */
							tempScore = tS.get(i - 1, j, k, l) + extractionInsDelScore;
							/*
							...
							/* Case other */
							tempScore = tS.get(i, j - 1, k, l) + extractionInsDelScore;
							/*
							 * Used if insertion and deletion costs are different
							 */
							// tempScore = tS.get(i-1, j, k, l) + delScore;
							...
							/* Case other */
							tempScore = tS.get(i, j, k, l - 1) + extractionInsDelScore;
							/*
							 * Used if insertion and deletion costs are different
							 */
							// tempScore = tS.get(i-1, j, k, l) + delScore;
							...
							tS.set(i, j, k, l, scoreMax);
							tSMemory.set(i, j, k, l, pathMax);
	...
	public ExtractionAlignments align(AnnotatedArray aa1, AnnotatedArray aa2, int desired_number_of_alignments, double minimum_score, int linesToProcess){		

		this.lineToProcess = linesToProcess;

		if(SABRE.getInstance().getParam()!= null && aa1.getNumberOfLines() >= lineToProcess && aa2.getNumberOfLines() >= lineToProcess){
			this.aa1 = aa1;
			this.aa2 = aa2;
	 ...
	 */
	private double[][] computeLocalScoreMatrix(List<Short> al1, List<Short> al2) {
		...
		if (size1 != 0 && size2 != 0) {
			...
			for (int i = 1; i <= size1; i++) {
				...
				for (int j = 1; j <= size2; j++) {
					...
					c_sub = score[i - 1][j - 1]
							+ similarity(al1.get(i - 1), al2.get(j - 1));
					c_del = score[i - 1][j] + extractionInsDelScore;

					score[i][j] = Math.max(
							Math.max(Math.max(c_ins, c_sub), c_del), 0.0);
	...
	private void traceback() {
		...
		TreeSet<Position4D> tracebackSeeds = getTSCompatiblePositions();

		//		if(tracebackSeeds.size() > 0)
		//			System.out.println(tracebackSeeds.size() + " seeds found");

		for(Position p: LPCALimitedLines.traceback){
			tracebackSeeds.add(new Position4D(p.getI(), p.getJ(), p.getK(), p.getJ()));
		}

		/* While there remain a valid position which have not been explored */
		while (tracebackSeeds.size() > 0) {

			Position4D lastPosition = tracebackSeeds.first();
			double positionScore = tS.get(lastPosition);

			// TODO voir si meilleur moyen de supprimer la premiere position
			tracebackSeeds.remove(lastPosition);

			if(positionScore >= alignments.minimumScore){

				/*
				 * Create a new ArrayList<Position4D> and remove the position from the
				 * seed's list
				 */
				ArrayList<Position4D> positions = new ArrayList<Position4D>();
				positions.add(lastPosition);

				//			System.out.println("Number of remaining seeds : " + tracebackSeeds.size());
				...
				while (isPathValid && !isPathOver) {
					...
					if (tSMemory.get(lastPosition) != -1) {
						...
						ArrayList<Position4D> newPositions = getNewPositions(lastPosition);

						if(doStupidTraceback){
...
//								System.out.println(i +"," + j + "," + k + "," + l);
								/* Case 8 */
								if (k > 0 && cS[i - 1][j - 1][k - 1][l-1] != 0 && j != 1
										&& rS[i - 1][j - 2][k - 1][l-1] != 0)
									tempScore = tS.get(i - 1, j - 1, k - 1, l-1)
									+ cS[i - 1][j - 1][k - 1][l-1]
											+ rS[i - 1][j - 2][k - 1][l-1];
						...
						/*
						 * If the new positions are valid (ie : if none of the new
						 * positions equal -1 in tsMemory)
						 */
				...
				/* Set all the position of the current path has already explored */
				...
				if (isPathValid) {
					...
					// TODO see why I need this to avoid the repetition of the
					// 1st position
					positions.remove(0);

					for (int i = 0; i < positions.size(); i++) {

						Short s1 = aa1.getAnnotations().get(positions.get(i).get(1) - 1).getValues().get(positions.get(i).get(0) - 1);
						Short s2 = aa2.getAnnotations().get(positions.get(i).get(1) - 1).getValues().get(positions.get(i).get(2) - 1);
					...
					alignments.addAlignment(new Alignment(p1, p2, positionScore));
					//					System.out.println("\tIn LPCA: number of alignments: " + alignments.currentNumberOfAlignments + " min score: " + alignments.minimumScore);
				}
	...
	private TreeSet<Position4D> getTSCompatiblePositions() {

		TreeSet<Position4D> result = new TreeSet<Position4D>(
				new Comparator<Position4D>() {

					/**
					 * Comparator to order the position by decreasing score. If
					 * two positions have the same score they are ordered
					 * according to their coordinates
					 * 
					 * @param p1
					 * @param p2
					 * @return -1 if p1 has a greater score in tS than p2 (or if
					 *         the score are the same and if p1 is greater than
					 *         p2) ; 0 if the position are the same ; 1
					 *         otherwise
					 */
					public int compare(Position4D p1, Position4D p2) {
						...
						if (score2 > score1)
							result = 1;
						/*
						 * If the score are the same, compare the position
						 * according to their coordinates
						 */
		 ...
		 */
		for (int j = aa1.getNumberOfAnnotationColumns(); j >= 0; j--)
			for (int i = this.lineToProcess; i >= 0; i--)
				for (int k = this.lineToProcess; k >= 0; k--) 
					for (int l = aa2.getNumberOfAnnotationColumns(); l >= 0; l--) {

						/* If the score is valid and better than <max> */
						if (tS.get(i, j, k, l) >= alignments.minimumScore) {
							Position4D pos = new Position4D(i, j, k, l);
							result.add(pos);
						}
					}
	...
	/**
	 * Get an ArrayList which at index 0 contains the previous position of <pos>
	 * in the current path. The other elements represents the position of the
	 * aligned annotations obtained by going from <pos> to it's previous
	 * position
	 * 
	 * @param position
	 *            Position4D from which we want the previous position
	 ...
	 *         aligned if it dooesn't occur in the other elements)
	 */
	private ArrayList<Position4D> getNewPositions(Position4D pos) {

		ArrayList<Position4D> result = new ArrayList<Position4D>();
		ArrayList<Position4D> tempPos = new ArrayList<Position4D>();
		Position4D temp = new Position4D(pos);
		...
		case 5:
			 ...
			 */
			result = getRowAlignedAnnotations(temp.getI() - 1, temp.getJ() - 1,
					temp.getK() - 1);
		...
		case 6:
			 ...
			 */
			result = getColAlignedAnnotations(temp.getI() - 1, temp.getJ() - 1,
					temp.getK() - 1, temp.getL()-1);
		...
		case 7:
			...
			if (result != null) {
				...
				tempPos = getColAlignedAnnotations(temp.getI() - 2,
						temp.getJ() - 1, temp.getK() - 2, temp.getL() - 1);
		...
		case 8:
			...
			if (result != null) {
				...
				tempPos = getRowAlignedAnnotations(temp.getI() - 1,
						temp.getJ() - 2, temp.getK() - 1);
	 ...
	 * @return
	 */
	private ArrayList<Position4D> getRowAlignedAnnotations(int i, int j, int k) {
		...
		if (i != k) {
			...
			double[][] simM = computeLocalScoreMatrix(subLine1, subLine2);
			...
			while (isValid && t < coord.size()) {
				...
				if (coord.get(t).getX() == coord.get(t).getY()) {
					...
					Position4D newPos = new Position4D(i + 1, coord.get(t).getX() + 1,
							k + 1, coord.get(t).getY());
	...
	public ArrayList<List<double[][]>> getColumnScoreMatrix() {
		return columnScoreMatrix;
	}

	public void setColumnScoreMatrix(ArrayList<List<double[][]>> ali_arg) {
	 ...
	 * @return
	 */
	private ArrayList<Position4D> getColAlignedAnnotations(int i, int j, int k, int l) {

		ArrayList<Position4D> result = new ArrayList<Position4D>();
		ArrayList<PointXMLSerializable> coord = traceback1D(aa1.getAnnotationColumn().get(j).getValues().subList(0, i + 1), aa2.getAnnotationColumn().get(j).getValues().subList(0, k + 1),
				columnScoreMatrix.get(j).get(l));
		...
		while (t < coord.size() && isValid) {
			...
			Position4D newPos = new Position4D(coord.get(t).getX() + 1, j + 1,
					coord.get(t).getY() + 1, l+1);
	...
	private ArrayList<PointXMLSerializable> traceback1D(List<Short> al1, List<Short> al2,
			double[][] simM) {
		...
		while (!isOver) {
			...
			if (simM[c.getX()][c.getY()] != 0) {
				...
				cSub = simM[c.getX() - 1][c.getY() - 1]
						+ similarity(al1.get(c.getX() - 1),al2.get(c.getY() - 1));
